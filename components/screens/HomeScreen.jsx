  ...
  const searchMovies = async (query) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/search/movie?api_key=${API_KEY}&language=en-US&query=${query}&page=1&include_adult=false`,
      )
      const data = await response.json()
      setMovies(data.results)
      setSearching(false)
  ...
  return (
    <View style={styles.container}>
      ...
      {loading || searching ? (
        ...
        <>
          <Text style={styles.sectionTitle}>{searchQuery ? "Search Results" : "Popular Movies"}</Text>
